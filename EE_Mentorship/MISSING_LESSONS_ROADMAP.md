...
- [ ] **Hàm chuẩn hóa xác suất Softmax (`cv_edge_soft_max.py`):** Phép tính exp(z_i) / sum(exp(z_j)) chuyển đổi đa lớp thành xác suất %.
...
- [ ] **Bản chất Trọng số W, Bias b và Sigmoid:** Tại sao 1 / (1 + exp(-Z)) ép tín hiệu về xác suất 0..1.
- [ ] **ReLU max(0, Z) ở Lớp Ẩn:** Định lý Xấp xỉ Vạn năng (Universal Approximation Theorem) và tại sao ReLU thắng Sigmoid.
- [ ] **Đạo hàm Lan truyền ngược Backpropagation:** Công thức Chain Rule dL/dW = 2 * (y_pred - y_true) * X.
- [ ] **Hàm Loss MSE vs BCE:** Phép phạt hóc hiểm của -log(y_pred) trong bài toán phân loại.
...
- [x] **Ring Buffer UART DMA FIFO:** Quản lý vòng đệm tròn tránh tràn RAM khi nhận dữ liệu telemetry 100Hz. (ĐÃ HOÀN THÀNH VÀ THẤU SUỐT 100% ✅)
